...
from src.utils.text_refinement import refine_text
...
# import ai_hawk.llm.prompts as prompts # Removed problematic import
...
class GPTAnswerer:
    ...
    def _clean_llm_output(self, output: str) -> str:
        """
        Clean and refine the LLM output text to improve quality.

        Args:
            output (str): The raw output from the LLM

        Returns:
            str: The cleaned and refined output
        """
        # First, do basic cleaning
        cleaned_text = output.replace("*", "").replace("#", "").strip()

        # Then apply advanced text refinement
        refined_text = refine_text(cleaned_text)
